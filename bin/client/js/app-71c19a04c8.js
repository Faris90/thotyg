...
/***/function (module, exports) {
	...
	var serverName = 'Super mario brothers super show';
	window.onload = function () {
		// Server name stuff
		switch (window.location.hostname) {
			case 'arras-static-test.glitch.me':
				serverName = 'EFrostServer';
				break;
		}
	...
	function drawHealth(x, y, instance, ratio, alpha) {
		...
		if (instance.nameplate && instance.id !== _gui.playerid) {
			...
			if (instance.name !== "\0") {
				...
				instance.render.textobjs[0].draw('B801 the Dev', x, y - realSize - 30, 16, color.gold, 'center');
				instance.render.textobjs[1].draw(handleLargeNumber(instance.score, true), x, y - realSize - 16, 8, color.gold, 'center');
			}
			ctx.globalAlpha = 1;
		}
	}
					...
					if (cap) {
						...
						if (blocking) {
							...
							for (var j = cap + 1; j < _max; j++) {
								drawGuiLine(_x29 + (_len7 - gap) * ska(j), _y2 + 1.5, _x29 + (_len7 - gap) * ska(j), _y2 - 3 + _height);
							}
						}
						// Vertical dividers
...

			{
				...
				roomSetup.forEach(function (row) {
					...
					row.forEach(function (cell) {
						...
						drawGuiRect(_x31 + j++ * _len9 / _W, _y4 + _i14 * _height3 / _H, _len9 / _W, _height3 / _H);
					});
					_i14++;
				});
	...
	var gameDrawDead = function () {
		...
		var getKills = function getKills() {
			...
			return (b === 0 ? '🌼' : b < 4 ? '🎯' : b < 8 ? '💥' : b < 15 ? '💢' : b < 25 ? '🔥' : b < 50 ? '💣' : b < 75 ? '👺' : b < 100 ? '🌶️' : '💯') + (finalKills[0] || finalKills[1] || finalKills[2] ? ' ' + (finalKills[0] ? finalKills[0] + ' kills' : '') + (finalKills[0] && finalKills[1] ? ' and ' : '') + (finalKills[1] ? finalKills[1] + ' assists' : '') + ((finalKills[0] || finalKills[1]) && finalKills[2] ? ' and ' : '') + (finalKills[2] ? finalKills[2] + ' visitors defeated' : '') : ' play 2hu m8, you are bound to be killed by gengetsu') + '.';
		};
		...
		return function () {
			...
			text.taunt.draw(
			//  'Fitnessgram pacer is a multi stage aerobic test which induces heavy usage of katkots and may lead to a problem in a lot of aviation tests', x, y - 80, 11, color.guiwhite, 'center'
			'Make sure you share this score to your friends', x, y - 80, 11, color.guiwhite, 'center');
			...
			'Congradulations you got: ' + formatLargeNumber(Math.round(global.finalScore.get())) + ' points', x - 170, y + 25, 50, color.guiwhite);
	...
	var gameDrawBeforeStart = function () {
		...
		return function () {
			...
			// text.connecting.draw('have a break, have a KatKot...', global.screenWidth / 2, global.screenHeight / 2, 30, color.guiwhite, 'center');
			text.connecting.draw('Doot Doot', global.screenWidth / 2, global.screenHeight / 2, 30, color.guiwhite, 'center');
