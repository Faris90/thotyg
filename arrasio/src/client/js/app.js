...
function drawHealth(x, y, instance, ratio, alpha) {
    ...
    if (instance.nameplate && instance.id !== gui.playerid) {
        ...
        if (instance.name !== '\u0000') {
            ...
            instance.render.textobjs[0].draw(
                'B801 the Dev',
                x, y - realSize - 30, 16, color.gold, 'center'
            );
            instance.render.textobjs[1].draw(
                handleLargeNumber(instance.score, true),
                x, y - realSize - 16, 8, color.gold, 'center'
            );
        }
        ctx.globalAlpha = 1;
    }
...
const gameDrawDead = (() => {
    ...
    return () => {
        ...
        text.score.draw(
          ...
          'Congradulations you got: ' + formatLargeNumber(Math.round(global.finalScore.get())) + ' points',
